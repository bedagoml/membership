...
class MpesaController extends Controller
{
    ...
    public function confirmationUrl()
    {
        ...
        if ($callbackData) {
            ...
            $tenant = Invoice::where('invoice_number', $payment->InvoiceNumber)->first();
            $failed_payment_sms = []; 
            if(!$tenant){
            ...
            $client_invoices = Invoice::where('id', $tenant->id)
                ->where('is_paid', 0)->get();
    ...
    private function paymentFailedSmsmFormat($notificationBody)
    {
        ...
        $message_text = sprintf($format, $mpesa_code, $name, $phone);
