...
class InitializeInvoices extends Command
{
     ...
     * @var string
     */
    protected $description = 'Generate invoices for all members in the annually.';
    ...
    public function handle()
    {
        ...
        $members = Tenant::where('membership', $subc->category)->get();
    ...
    public function setTotalPayable($month)
    {
        ...
        foreach ($invoices as $invoice) {
            Invoice::where('house_id', $invoice->house_id)
                ...
                ->where('rent_month', $month)
                ->update(['total_payable' => ($invoice->rent  + $invoice->carryforward + $invoice->electricity_bill + $invoice->water_bill + $invoice->compound_bill + $invoice->litter_bill + $invoice->security + $invoice->other_bill + $invoice->deposit_paid)]);
